...
- **at-a-glance** — A 2–3 sentence plain-language summary filling the `{at-a-glance}` placeholder in the `.plan-glance` block. Placement (sibling immediately after `div#objective`), the extractor rationale, and the job split with the objective are defined once in the **At a glance** rule in Step 2 — follow that rule.
...
- Render the step list as numbered cards with an expandable verify disclosure whose summary reads *How to check this worked* (`<details><summary>How to check this worked</summary>…</details>`)
...
- Include an **At a glance block** (the `.plan-glance` element) as a **sibling immediately after `div#objective`** and before the `.plan-implement` row — never nested inside `#objective` (see the At a glance rule in Step 2 for why). Fill its `{at-a-glance}` placeholder with the 2–3 sentence plain-language summary written in Step 2. Keep the block's visible "At a glance" label and its accessible-name wiring (`aria-labelledby` / `aria-label`) exactly as the skeleton ships them.
